    ...
    >
      <view class="specification-wrapper">
        <scroll-view class="specification-wrapper-content" scroll-y="true">
          <view class="specification-header">
            ...
            <view class="specification-right">
              <view :style="{ color: themeColorFn('priceColor') }" class="price-content">
                <text class="sign">¥</text>
                <text :class="priceCom.length > 16 ? 'price2' : ''" class="price">
                  {{ priceCom }}
                </text>
              </view>
              <view v-if="!hideStock" class="inventory"> {{ stockText }}：{{ stockCom }} </view>
              <view v-else class="inventory"></view>
              <view v-show="isManyCom" class="choose"> 已选：{{ selectArr.join(' ') }} </view>
            </view>
          </view>
        ...
        <view v-if="showClose != false" class="close" @click="close('close')">
          <image :src="closeImage" class="close-item"></image>
        </view>
      </view>
